$(function () {
  $('input[name="checkContact[]"][value="耐震補強"]').change(function () {
    if ($(this).is(":checked")) {
      $('input[name="施工予定の建物の構造[]"]')
        .addClass("validateMinCheckbox")
        .addClass("validate[minCheckbox[1]]")
        .attr("data-prompt-position", errTxtPosition);
    } else {
      $('input[name="施工予定の建物の構造[]"]')
        .removeClass("validateMinCheckbox")
        .removeClass("validate[minCheckbox[1]]")
        .removeAttr("data-prompt-position");
    }
  });

  $('input[name="checkContact[]"][value="LSP工法"]').change(function () {
    if ($(this).is(":checked")) {
      $('input[name="LSP工法施工予定"], input[name="LSP工法施工予定住所"]')
        .addClass(".validateRequired")
        .addClass("validate[required]")
        .attr("data-prompt-position", errTxtPosition);
    } else {
      $('input[name="LSP工法施工予定"], input[name="LSP工法施工予定住所"]')
        .removeClass(".validateRequired")
