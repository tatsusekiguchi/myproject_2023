...
// -------------------------------------------------------------------
// 管理画面の「投稿」を「お知らせ・イベント」に変更
// -------------------------------------------------------------------
function change_post_menu_label() {
	global $menu;
	global $submenu;
	$menu[5][0] = 'お知らせ・イベント';
	$submenu['edit.php'][5][0] = 'お知らせ・イベント一覧';
	$submenu['edit.php'][10][0] = '新しいお知らせ・イベント';
	$submenu['edit.php'][16][0] = 'タグ';
}
function change_post_object_label() {
	global $wp_post_types;
	$labels = &$wp_post_types['post']->labels;
	$labels->name = 'お知らせ・イベント';
	$labels->singular_name = 'お知らせ・イベント';
	$labels->add_new = _x('追加', 'お知らせ・イベント');
	$labels->add_new_item = 'お知らせ・イベントの新規追加';
	$labels->edit_item = 'お知らせ・イベントの編集';
	$labels->new_item = '新規お知らせ・イベント';
	$labels->view_item = 'お知らせ・イベントを表示';
	$labels->search_items = 'お知らせ・イベントを検索';
...
function new_post_type1(){
	register_post_type(
	  ...
	  'label' => 'キャストカテゴリー',
	  'labels' => array(
	  'popular_items' => 'よく使うキャストカテゴリー',
	  'edit_item' => 'キャストカテゴリーを編集',
	  'add_new_item' => '新規キャストカテゴリーを追加',
	  'search_items' => 'キャストカテゴリーを検索',
	  ),
	  'public' => true,
	  'hierarchical' => true,
	  'rewrite' => array('slug' => 'cast_cat')
	  )
	);
}
