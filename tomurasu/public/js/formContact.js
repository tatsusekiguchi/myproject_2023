$(function () {
  ...
  $(".inputFile01 input[type=file]").on("change", function () {
    ...
    $(".inputFile01 + .fileTxt .fileName").text(file.name);
  });
  $(".inputFile02 input[type=file]").on("change", function () {
    var file = $(this).prop("files")[0];
    $(".inputFile02 + .fileTxt .fileName").text(file.name);
  });
  $(".inputFile03 input[type=file]").on("change", function () {
    var file = $(this).prop("files")[0];
    $(".inputFile03 + .fileTxt .fileName").text(file.name);
  });
  $(".inputFile04 input[type=file]").on("change", function () {
    var file = $(this).prop("files")[0];
    $(".inputFile04 + .fileTxt .fileName").text(file.name);
  });
  $(".inputFile05 input[type=file]").on("change", function () {
    var file = $(this).prop("files")[0];
    $(".inputFile05 + .fileTxt .fileName").text(file.name);
  });
  ...
  if (formDataArr.length > 0) {
    ...
    formDataArr.map(function (item) {
      ...
      switch (item.name) {
        ...
        case "postal_04":
          postal02 = item.value ? "-" + item.value : "";
          formData.append("お届け先の郵便番号", postal01 + postal02);
          break;
        // 生年月日
        // ▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△▼△
        case "year":
