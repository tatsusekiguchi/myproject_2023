...
//}
// Add by KAGOYA
if ( $step < 3 && is_blog_installed() ) {
	display_header();
	echo '<h1>'.__('Already Installed').'</h1><p>';
	echo __('You appear to have already installed WordPress. To reinstall please clear your old database tables first.').'</p></body></html>';
	printf('<div align="center"><div style="float:left;"><a href="%s">&lt;&lt; '. mb_convert_encoding("wp-config.php ファイルの設定へ戻る", "UTF-8", "auto") .'</a></div>','./setup-config.php?step=1');
	printf('<div style="float:right;"><a href="%s?step=3">'. mb_convert_encoding("インストールを完了する", "UTF-8", "auto") .' &gt;&gt;</a></div>',basename(__FILE__));
	printf('<div><a href="%s">&gt;&gt; '. mb_convert_encoding("再実行します", "UTF-8", "auto") .' &lt;&lt;</a></div></div></body></html>',basename(__FILE__));
	exit();
}
...
<?php
$error = 0;
if(!@unlink('./install.php')) $error = 1;
if(!@chmod('../wp-config.php',0444)) $error = 1;
if($error){
                $error_proc = "ファイルの削除、または権限変更に失敗しました。ご自身で次の作業を行ってください。"
                        ."<ul><li>インストールディレクトリ/wp-admin/install.phpを削除してください。</li>\n"
                        ."<li>インストールディレクトリ/wp-config.phpの権限を0444に変更してください。</li></ul>";
?>
<p class="message"><?php printf( __( '<strong>ERROR</strong>: %s' ), mb_convert_encoding($error_proc, "UTF-8", "auto")); ?></p>
<?php
}
?>
