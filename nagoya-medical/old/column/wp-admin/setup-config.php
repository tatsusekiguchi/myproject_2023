...
//Add by KAGOYA
if ($step === -1 && file_exists( ABSPATH . 'wp-config.php' ) ) {
        if (filesize( ABSPATH . 'wp-config-sample.php' ) != filesize( ABSPATH . 'wp-config.php' ) ) {
    wp_die(mb_convert_encoding('<p>' . sprintf(__( " 'wp-config.php' ファイルは既に作成済みです。このファイル内の設定項目をリセットする必要がある
のなら、<a href='%s'>こちら</a>をクリックしてください。その後で <a href='%s'>インストール</a>を実行してください。設定項目をリセットする必要がない
場合は 'wp-config.php' の権限を444へ変更して下さい。"), 'setup-config.php?step=1', 'install.php') . '</p>', "UTF-8", "auto"));
        }
}
