...
$(document).ready(function () {
  ...
  if ($(".blogPanel--list").length > 0) {
    ...
    // 'wg[wgc-1701688112317_cate]'という名前のクエリパラメータを取得
    var categoryParam = urlParams.get("wg[wgc-1701688112317_cate]");

    // クエリパラメータが存在しない場合、"allItem"のaタグにクラスを追加
    if (!categoryParam) {
      $(".allItem > a").addClass("active");
    } else {
      // クエリパラメータの値に基づいて該当するaタグにクラスを追加
      $(
        ".webgene-item > a[href*='wg%5Bwgc-1701688112317_cate%5D=" +
          categoryParam +
          "']"
      ).addClass("active");
    }
