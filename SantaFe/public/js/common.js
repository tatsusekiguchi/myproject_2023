...
$(document).ready(function () {
  ...
  if ($(".section__beforeAfter").length > 0) {
...
function setAccord() {
  ...
  $(".accord .close").click(function () {
    $(this).parent().slideUp();
    $(this).parent().prev("dt").removeClass("active");
  });
}
...
function keyvSubSlider() {
  if ($(".topSubKv").length) {
    const HEADER_ELEM = $(".topSubKv");
    const FADE_SPEED = 2500;
    const SWITCH_DELAY = 8000;

    // 要素作成
    if (!HEADER_ELEM.children().hasClass("kvSubBox")) {
      let keyBoxElem = "";
      keyBoxElem += '<div class="kvSubBox">';
      keyBoxElem += '<div class="kvBg kv02"></div>';
      keyBoxElem += '<div class="kvBg kv03"></div>';
      keyBoxElem += '<div class="kvBg kv01"></div>';
      keyBoxElem += "</div>";
